        ...
        <section ref={sectionRef} className="section-padding bg-ivory relative overflow-hidden">
            ...
            <div className="container-custom relative z-10">
                ...
                <div className="text-center mb-24 md:mb-32">
                    <div className="education-header-reveal reveal-hidden inline-flex items-center gap-3 mb-6">
                    ...
                    <h2 className="education-header-reveal reveal-hidden font-display text-6xl md:text-8xl text-charcoal mb-10 leading-[0.85]">
                        The Physics <br />
                        <span className="italic font-light text-gradient drop-shadow-sm">Behind the Glow</span>
                    </h2>
                    <p className="education-header-reveal reveal-hidden max-w-2xl mx-auto text-soft-gray font-sans text-lg md:text-xl leading-relaxed">
                ...
                {/* Steps Narrative */}
                <div className="grid grid-cols-1 md:grid-cols-5 gap-10 lg:gap-14 mb-32 md:mb-48">
                    {steps.map((step, index) => (
                        <div key={index} className="step-item reveal-hidden flex flex-col items-center text-center group cursor-default">
                            <div className="relative mb-10">
                                <div className="w-24 h-24 rounded-[40%] bg-white border border-charcoal/5 flex items-center justify-center shadow-premium group-hover:bg-charcoal transition-all duration-700 transform group-hover:-translate-y-3 group-hover:rotate-[15deg]">
                                    <step.icon className="w-10 h-10 text-deep-rose group-hover:text-warm-gold transition-all duration-500" />
                                </div>
                                <div className="absolute -bottom-3 -right-3 w-10 h-10 rounded-full bg-ivory text-charcoal font-black text-[10px] flex items-center justify-center border border-charcoal/5 shadow-xl group-hover:bg-deep-rose group-hover:text-white transition-colors">
                                    {String(index + 1).padStart(2, '0')}
                                </div>
                                {index < steps.length - 1 && (
                                    <div className="hidden md:block absolute top-12 -right-4 translate-x-1/2 w-12 h-px border-t border-dashed border-charcoal/10 pointer-events-none" />
                                )}
                            </div>
                            <h3 className="font-display text-2xl text-charcoal mb-4 group-hover:text-deep-rose transition-colors duration-500">{step.title}</h3>
                            <p className="text-soft-gray text-sm leading-relaxed font-sans group-hover:text-charcoal transition-colors duration-500">
                                {step.description}
                            </p>
                        </div>
                    ))}
                ...
                <div className="quiz-section-reveal opacity-0 scale-95 w-full max-w-5xl mx-auto">
                    <div className="glass-card p-12 md:p-20 relative overflow-hidden group/card shadow-[0_40px_100px_rgba(0,0,0,0.08)]">
                            ...
                            <div className="w-full lg:w-1/3 flex flex-col items-center">
                                <div className="relative w-48 h-48 flex items-center justify-center mb-8">
                                    <svg className="w-full h-full -rotate-90">
                                        <circle
                                            cx="96" cy="96" r="88"
                                            className="fill-none stroke-charcoal/5 stroke-[6]"
                                        />
                                        <circle
                                            cx="96" cy="96" r="88"
                                    ...
                                    <div className="absolute inset-0 flex flex-col items-center justify-center">
                                        <HelpCircle className="w-12 h-12 text-warm-gold mb-2 animate-pulse-soft" />
                                        <span className="text-3xl font-display font-medium text-charcoal">{quizStep + 1}<span className="text-charcoal/20 mx-1">/</span>3</span>
                                    </div>
                                </div>
                                <div className="text-center">
                                    <span className="text-[10px] font-black uppercase tracking-[0.5em] text-deep-rose">Digital Diagnostic</span>
                                    <p className="text-soft-gray/60 font-sans text-[10px] mt-2 italic">Real-time candidacy mapping</p>
                                </div>
                            </div>

                            {/* Tool Interface */}
                            <div className="w-full lg:w-2/3 min-h-[300px] flex flex-col justify-center">
                                {!quizResult ? (
                                    <div className="space-y-10">
                                        <div className="space-y-4">
                                            <span className="text-[11px] font-black uppercase tracking-[0.3em] text-charcoal/40">Diagnostic Stage {quizStep + 1}</span>
                                            <h4 className="font-display text-4xl md:text-5xl text-charcoal leading-tight">
                                        ...
                                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                                            {(quizStep === 0 ? ["Facial Contour", "Full Body Ritual", "Precision Zones", "Intimate Areas"] :
                                                ...
                                                <button
                                                    ...
                                                    className="group flex items-center justify-between p-6 bg-white rounded-3xl border border-charcoal/5 hover:border-deep-rose/30 hover:bg-deep-rose/5 transition-all duration-500 transform hover:scale-[1.03] hover:shadow-xl font-sans font-bold text-sm tracking-wide text-charcoal/80"
                                                >
                                                    <span>{opt}</span>
                                                    <div className="p-2 rounded-full bg-charcoal/5 group-hover:bg-deep-rose group-hover:text-white transition-all">
                                    ...
                                    <div className="space-y-10 animate-fade-in-up">
                                        <div className="inline-flex items-center gap-3 bg-deep-rose/5 text-deep-rose px-6 py-3 rounded-full text-[10px] font-black uppercase tracking-[0.3em] border border-deep-rose/10 pointer-events-none">
                                            <CheckCircle2 className="w-4 h-4 animate-pulse-soft" />
                                            <span>Synthesis Complete: Match Verified</span>
                                        </div>

                                        <div className="space-y-6">
                                            <h4 className="font-display text-5xl md:text-6xl text-charcoal leading-none">
                                        ...
                                        <div className="flex flex-wrap gap-6 pt-4">
                                            ...
                                            <button
                                                ...
                                                className="px-10 py-5 rounded-full border border-charcoal/10 text-[11px] font-black uppercase tracking-[0.3em] text-soft-gray hover:bg-white hover:text-charcoal transition-all"
                                            >
